...
| `polar2_pg_wave5_exploit` | polar2 | `polar2_pg_wave5_exploit_policy_gradient.log` | In progress | Resumed from `polar2_pg_wave5_checkpoint_18000.pth` (131 avg) with entropy=0, death penalty (-0.1 over 60 frames), wave 5 curriculum. Testing whether removing entropy tightens up the noisy action selection without causing policy collapse. |
